...
class PtEngine
{
    ...
    /**
     * Aşağıda, alert için parse işlemi yapar
     */
    public function parseAlert()
    {
        $this->view = preg_replace_callback('/@alert\(\'(.*?)\'\)(.*?)@endalert/s', function($result) {
            return '<?php if (isset($_SESSION[\'alert\'][\'' . $result[1] . '\'])): ?> ' . $result[2] . ' <?php endif; ?>';
        }, $this->view);
    }

    /**
     * Aşağıda, mixin dosyası eklemek için parse işlemi yapar
     */
    public function parseUseMixin()
    {
        $this->view = preg_replace_callback('/@usemixin\(\'(.*?)\'\)/', function($result) {
            $result[1] = str_replace('.', '/', $result[1]) . '.php';
            return '<?php require_once ROOT_DIR . \'/views/' . $result[1] . '\' ?>';
        }, $this->view);
    }

    /**
     * Aşağıda, mixin dosyasının içindeki fonksiyonu çalıştırmak için parse işlemi yapar
     */
    public function useMixin()
    {
        $this->view = preg_replace_callback('/@usemixin\(\'(.*?)\'\)/', function($result) {
            $result[1] = str_replace('.', '/', $result[1]) . '.php';
            return '<?php require_once ROOT_DIR . \'/views/' . $result[1] . '\' ?>';
    ...
    /**
     * PARSE CSRF TOKENS FOR SECURITY
     */
    public function parseCSRF()
    {
        $this->view = preg_replace_callback('/@csrf/', function ($yieldName) {
            return '<input name="_csrf" value="<?=Session::get_csrf() ?>" type="hidden" />';
        }, $this->view);
    }
